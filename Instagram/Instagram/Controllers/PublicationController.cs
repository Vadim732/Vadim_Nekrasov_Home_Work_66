...
public class PublicationController : Controller
{
    ...
    public async Task<IActionResult> Follow(int followingId)
    {
        ...
        if (following == null)
        {
            return Json(new { success = false, message = "User not found :c" });
        }
        ...
        return Json(new { success = true, followingId });
    }
    ...
    public async Task<IActionResult> DeletePostConfirmed(int publicationId)
    {
        var user = await _userManager.GetUserAsync(User);
        var publication = await _context.Publications.FirstOrDefaultAsync(p => p.Id == publicationId);
        if (publication == null || publication.UserId != user.Id)
        {
            return Json(new { success = false, message = "Publication not found or access denied." });
        }
